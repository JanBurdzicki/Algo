...
// TODO: naklepane zgodnie z CP Algo, ale nie testowane, wiec moze byc bledne

const int INF = 1e9 + 5;

struct Floyd_Warshall_Ujemne_Wagi
{
	...
	Floyd_Warshall_Ujemne_Wagi(vector <vector <bool>>& czy_istnieje_krawedz, vector <vector <int>>& wagi_krawedzi, int n)
	{
		for(int i = 1; i <= n; i++)
		{
			for(int j = 1; j <= n; j++)
...
int main()
{
	...
	Floyd_Warshall_Ujemne_Wagi floyd_warshall(czy_istnieje_krawedz, wagi_krawedzi, n);
	...
	for(int numer_zapytania = 0; numer_zapytania < t; numer_zapytania++)
	{
		...
		if(wynik == Floyd_Warshall_Ujemne_Wagi::INF)
		{
			cout << "-1\n";
		}
